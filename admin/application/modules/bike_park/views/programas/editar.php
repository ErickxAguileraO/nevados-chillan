    ...
    <form action="#" method="post" id="form-editar" >
        <div class="row" style="margin-top:30px; margin-bottom:30px;">
            ...
            <input type="hidden" class="form-control validate[required]" id="codigo" name="codigo" value="<?=$programa->codigo?>"/>
                ...
                <label>Monto (*) </label>
                <input type="text"   class="form-control validate[required]" name="editar_monto[]" value="<?=$op->monto?>"/>
                <label>Resumen (*) </label>
                <textarea   class="form-control" name="editar_resumen[]" rows="3" ><?=$op->resumen?></textarea>
                <label>Orden</label>
                <input type="number"   name="editar_orden[]" min="0" class="form-control validate[numeric]"  value="<?=$op->orden?>"/>
			...
			<div class="col-xs-12">
				<div class="text-left" style="margin-top:20px;">
					<a href="/bike-park/programas/" class="btn btn-can">Cancelar</a>
		  ...
		  buttons: [
			{addClass: 'btn btn-primary', text: 'Aceptar', onClick: function($noty) {
				...
				$.ajax({
					...
					data: "codigo="+codigo,
					dataType: "json",
					url: '/bike_park/programas/eliminar_opcion/',
