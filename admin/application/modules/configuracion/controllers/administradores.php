...
class Administradores extends CI_Controller {
	...
	private $modulo = 61, $modulo_seccion = 62, $modulo_seccion_usuario = 63;
	...
	public function agregar(){
        ...
        if($this->input->post()){
            ...
            $codigo = $id->adm_codigo;
            
            #agrega las secciones a las que tiene permiso
            if($secciones = $this->input->post('secciones')){
                foreach($secciones as $aux){
                    unset($datos);
                    $datos['adms_seccion'] = $aux;
                    $datos['adms_administrador'] = $codigo;
                    
                    $this->ws->insertar($this->modulo_seccion_usuario,$datos);
                }
            }
            ...
            #secciones
            $contenido['secciones'] = $this->ws->listar($this->modulo_seccion,"seu_estado = 1");
            
    		#view
    		$this->layout->view('administradores/agregar',$contenido);
        }
	}
    ...
    public function editar($codigo){
        ...
        if($this->input->post()){
            ...
            #agrega las secciones a las que tiene permiso
            $this->ws->eliminar($this->modulo_seccion_usuario,"adms_administrador = $codigo");
            if($secciones = $this->input->post('secciones')){
                foreach($secciones as $aux){
                    unset($datos);
                    $datos['adms_seccion'] = $aux;
            ...
            #secciones
            $contenido['administrador']->secciones = $this->ws->listar($this->modulo_seccion_usuario,"adms_administrador = $codigo");
