...
class Secciones extends CI_Controller {
	...
	function __construct(){
        ...
        $this->img->upload_dir = '/imagenes/modulos/historia/secciones/';
	...
	public function index(){
        ...
        #js
        $this->layout->js('/js/sistema/historia/secciones/index.js');
        
        $where = $and = "";
        $url = "";
        
        $where = "secc_tipo_seccion = 15";
        $and = " and ";
		...
		$config['base_url'] = '/historia/secciones/';
		$config['per_page'] = 20;
	...
	public function agregar(){
        ...
        if($this->input->post()){
            ...
            if($_FILES['imagen']['name']){
                ...
                //$config['file_name'] = ($this->input->post('nombre_imagen_adjunta'))?slug($this->input->post('nombre_imagen_adjunta')).'-'.time()+rand(1,543543453).'.'.$extension:basename($imagen['name']);
            ...
            if($grandes){
                foreach($grandes as $k=>$aux){
                    if($aux){
                        ...
                        $datos['sec2_seccion'] = $id->secc_codigo; 
    ...
    public function editar($codigo){
        ...
        if($this->input->post()){
          ...
          /*  if($this->input->post('ruta_interna_2'))
            ...
            #registro
            if($contenido['seccion'] = $seccion = $this->ws->obtener($this->modulo,"secc_codigo = '$codigo' and secc_tipo_seccion = 15"));
            else show_error('');
