				...
				that.cropControlRemoveCroppedImage.on('click',function(){ 
					...
					if(that.options.outputUrlId !== ''){
						if($('#'+that.options.outputUrlId).val() != "" ){
							...
							$('#'+that.options.outputUrlId).remove();
						}
					}
					...
					if(that.options.outputUrlIdGr !== ''){
						if($('#'+that.options.outputUrlIdGr).val() != "" ){
							...
							$('#'+that.options.outputUrlIdGr).remove();
						}
					}
					...
					if(that.options.outputUrlIdIn !== ''){
						if($('#'+that.options.outputUrlIdIn).val() != "" ){
							...
							$('#'+that.options.outputUrlIdIn).remove();
						}
					}
                    
                    if(that.options.outputUrlId !== ''){
						//if($('#'+that.options.outputUrlId).val() != "" ){
                            var codigo = that.options.outputUrlId.split('-');
                            codigo = codigo[3];
        				    $("#img"+codigo).parent().remove();
                        //}
                    }
                    
                    if(that.options.cargaContenedorDelete){
                        var cargar = [];
                        cargar.push(that.options.idContenedor);
                ...
                type: 'POST'
					...
					if(response.status=='success'){
						...
						if(response.ruta_grande)
                            if(that.options.outputUrlIdGr !== ''){	$('#'+that.options.outputUrlIdGr).val(response.ruta_grande);	}
