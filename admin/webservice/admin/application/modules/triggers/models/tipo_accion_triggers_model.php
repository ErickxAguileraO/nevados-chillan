...
class Tipo_accion_triggers_model extends CI_Model {
    
    private $tabla = 's_tipo_accion_triggers';
    function __construct(){
        parent::__construct();
    }

    public function obtener_por_codigo($codigo){
        return $this->obtener(array('tatr_codigo'=>$codigo));
	}
	...
	public function obtener($where){
		...
		if($aux){
            $obj = new stdClass();
			$obj->codigo = $aux->tatr_codigo;
			$obj->nombre = $aux->tatr_nombre;
